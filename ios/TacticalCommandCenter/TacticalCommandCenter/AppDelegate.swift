...
class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate, MessagingDelegate {
    ...
    func application(_ application: UIApplication,
        ...
        // Personal (free) Apple developer accounts cannot sign the
        // `com.apple.developer.usernotifications.critical-alerts` entitlement;
        // requesting .criticalAlert without it crashes the app at launch on
        // some iOS versions. Only include it if the entitlement is actually
        // wired into the target build settings (CODE_SIGN_ENTITLEMENTS set).
        // For paid teams, add "-D HAVE_CRITICAL_ALERTS" to Swift Compiler →
        // "Other Swift Flags" to re-enable.
        #if HAVE_CRITICAL_ALERTS
