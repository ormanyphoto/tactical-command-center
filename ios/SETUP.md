...
- **Location not tracking at all (user is visible, map doesn't move)**: this is almost always a missing `NSLocationWhenInUseUsageDescription` in the Info.plist. Without it, iOS **silently refuses** the permission prompt — no dialog appears and `startUpdatingLocation()` delivers zero updates. The project uses `GENERATE_INFOPLIST_FILE = YES`, so the keys live in the build-settings as `INFOPLIST_KEY_NSLocationWhenInUseUsageDescription`, `INFOPLIST_KEY_NSLocationAlwaysAndWhenInUseUsageDescription`, and `INFOPLIST_KEY_UIBackgroundModes = "location remote-notification fetch"`. After adding these, **delete the app from the device first** — a prior `.denied` cache from a broken build will not re-prompt. Reinstall → iOS shows the prompt the first time the map loads.
- **Location updates stop in background**: verify `UIBackgroundModes` → `location` is in the generated plist AND `allowsBackgroundLocationUpdates = true` is taking effect in `LocationManager.swift` (it's gated on `hasBackgroundLocationMode()`). The user must have granted "Always" permission, not just "While Using".
